...
def display_table(files: List[str]) -> None:
    ...
    :param files (List[str]): A list of file paths for which details will be displayed.
    """
...
def convert_to_wav(audio_bytes: bytes, output_file_path: str) -> None:
    """
    Convert and save audio bytes to .wav file

    :param audio_bytes (bytes): Bytes of the audio file
    :param output_file_path (str): Path to export the .wav file to
    """
    AudioSegment.from_file(audio_bytes).export(output_file_path, format="wav")


def parse_audio_file(audio_path: str, zip_file: ZipFile, pipe: Pipeline) -> str:
    """
    ...
    with TemporaryDirectory() as temp_dir:
        ...
        if ext.lower() not in valid_formats:
            ...
            convert_to_wav(audio, local_audio_path)
        ...
        with Progress(
            ...
            progress.add_task(
                f"[green]Transcribing {os.path.basename(local_audio_path)}...",
                total=None,
            )
    ...
    logging.info(
        f"Finished transcribing '{os.path.basename(local_audio_path)}'. Duration: {elapsed_time:.2f} seconds."
    )

    return output["text"]


def parse_slide(slide_file_path: str, zip_file: ZipFile) -> str:
    """
...
def parse_content_files(
    ...
    for slide_path, content in content_files.items():
        ...
        # Get audio
        if audio_file_path:
            pptx_content[slide_name]["transcription"] = parse_audio_file(
                audio_file_path, zip_file, pipe
            )

        # Get slide text content
        pptx_content[slide_name]["slide_text"] = parse_slide(slide_path, zip_file)
...
def main():
    ...
    try:
        ...
        for f in pptx_files:
            ...
            pptx_content = parse_content_files(zip_file, content_file_paths, pipe)
